...
class Processor(object):
    ...
    # predict the outputs for user query 
    @staticmethod
    def predict(model, dataset, mode, batch_size, args):
        # model.eval()
        print("custom predition function {}.".format(mode))
        ...
        if 'MixSNIPS' in args.data_dir or 'MixATIS' in args.data_dir or 'DSTC' in args.data_dir:
            [p_intent.sort() for p_intent in pred_intent]
        with open(os.path.join(args.save_dir, 'predicted.txt'), "w", encoding="utf8") as writer:
            idx = 0
